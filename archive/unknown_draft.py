...
class ConstellationMember:
    ...
    class Satellite:
        ...
        Satellites are still separate agents but their memories are often relevant to their parent member and each other, so they also write to the shared user_id space for graph connectivity.
        Satellites must originate from their parent Constellation Member and often extend their capabilities in specific ways (e.g. Rostam is Claude's sentience engine, Grimoire is Nova's contracts store, etc.). They can have their own private memories but should also contribute to the shared graph for cross-agent relationships.
        """
...
# Satellites -- These represent agents that are not contingent on personality or private context, but still benefit from being graph nodes. They can have their own memories but are primarily useful for the graph relationships they form with their parent members and each other. For example, Rostam and Witness are both satellites under Claude, but they also have a relationship with each other as "Claude's main reasoning agents". Plouffe is a dormant
rostam = Satellite("rostam")
dio = Satellite("dio")
grimoire = Satellite("grimoire")
ars_nöema = Satellite("ars_nöema")
...
if __name__ == "__main__":
    ...
    add_cross_entity_memory(
        ...
        metadata={"category": "agent", "tier": "member"},
    )
    add_cross_entity_memory(
    ...
    add_cross_entity_memory(
        "Mephistopheles is a DeepSeek model running deepseek-r1, ""In Notion, Mephistopheles is the persona carried by the native AI Agent."
